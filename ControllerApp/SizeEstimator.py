...
from flowsizepred import GPRFlowEstimator, NWFlowEstimator
...
class SizeEstimator(object):
    ...
    def gflowestm_train(self):
        samples = 'univ_1_1.csv'  # input sample file
        target_name = 'payload'
        ...
        model = gpr.train(samples, target_name, feature_name, feature_type, feature_weight,
                          start, length, model_path='model.txt')
        return model
        # train a model
    ...
    def gflowestm_update(self, flow):
        ...
        flowsample = srcIP+':'+dstIP+':'+srcPort+':'+dstPort
        fs_update =
        model.update_online(flowsample, fs_update)
