...
class GPRFlowEstimator:
    def train(self, data_path, target_name, feature_name, feature_type, feature_weight=[], start=0, length=-1,
              model_path='GPR_model.txt', max_iter=100, rglr=1e-1, learning_rate=0.005):
        ...
        idx_sr, target1, target_mean1, target_std1, D_nm, D_mm, theta_gp1, eta_gp1, Theta_gp1, Eta_gp1, LLH1 = self.gp_training_cplt_2(
            data, -1, feature_type, feature_weight, max_iter, rglr, learning_rate)
        idx_sr_data, target2, target_mean2, target_std2, D_nm, D_mm, theta_gp2, eta_gp2, Theta_gp2, Eta_gp2, LLH2 = self.gp_training_cplt_2(
            data, -1, feature_type, feature_weight, max_iter, rglr, learning_rate, idx_sr)

        theta1 = np.exp(theta_gp1)
        delta1 = []
        for i in range(0, num_of_feature):
            delta1.append(np.exp(eta_gp1[i]))
        K_nm1 = GaussOperation.kernel_gauss_gp_cplt(D_nm, theta1, delta1)
        K_mm1 = GaussOperation.kernel_gauss_gp_cplt(D_mm, theta1, delta1)
        Lambda1 = np.real(
            inv(np.dot(np.transpose(K_nm1), K_nm1) + rglr * (K_mm1 + rglr * np.identity(len(idx_sr), dtype=float))))
        ...
        K_nm2 = GaussOperation.kernel_gauss_gp_cplt(D_nm, theta2, delta2)
        K_mm2 = GaussOperation.kernel_gauss_gp_cplt(D_mm, theta2, delta2)
        Lambda2 = np.real(
            inv(np.dot(np.transpose(K_nm2), K_nm2) + rglr * (K_mm2 + rglr * np.identity(len(idx_sr), dtype=float))))
        ...
        model.setTargetStd1(target_std1)
        model.setTargetStd2(target_std2)
    ...
    def gp_training_cplt_2(self, input, target_pos, feature_type, feature_weight, max_iter, rglr, lda_learning,
                           idx_sr_pre=[]):
        ...
        D_nm = GaussOperation.gp_k_cplt(data, data_sr, feature_type)

        D_mm = GaussOperation.gp_k_cplt(data_sr, data_sr, feature_type)

        LLH = [self.gp_likelihood_cplt2(target, D_nm, D_mm, theta, eta, rglr)]
        ...
        while curr_iter < max_iter:
            ...
            Old_LLH = self.gp_likelihood_cplt2(target, D_nm, D_mm, pre_theta, pre_eta, rglr)

            for i in range(0, num_of_feature):
                pre_eta_stepsize = pre_eta[:]
                pre_eta_stepsize[i] = pre_eta_stepsize[i] + stepsize
                eta[i] = pre_eta[i] - lda_learning * (
                    self.gp_likelihood_cplt2(target, D_nm, D_mm, pre_theta, pre_eta_stepsize, rglr) - Old_LLH) / stepsize

            Eta[:, curr_iter] = np.array(eta).reshape(num_of_feature, 1)[:, 0]

            theta = pre_theta - lda_learning * (
                self.gp_likelihood_cplt2(target, D_nm, D_mm, pre_theta + stepsize, pre_eta, rglr) - Old_LLH) / stepsize
            Theta.append(theta)
        ...
        return idx_sr, target, target_mean, target_std, D_nm, D_mm, theta, format_eta, format_Theta, Eta, LLH
    ...
    def update(self, data_path, model_path, start=0, length=-1, overwrite=True):
        ...
        target_mean1 = old_model.getTargetMean1()
        target_std1 = old_model.getTargetStd1()
        pre_str = old_model.getSampleSize()
        for k in range(0, length):
            y_new = update_data[k, -1].astype(float)
            y_new = np.array(y_new)
            y_new = np.reshape(y_new, (1, 1))
            sample_data = [update_data[k, 0:-1]]
            D_star = GaussOperation.gp_k_cplt(sample_data, old_model.getIdxSr(), old_model.getFeatureType())
            K_star = GaussOperation.kernel_gauss_gp_cplt(D_star, theta1, eta1)

            K = K_star.transpose()
            alpha1, Lambda1, target_mean1, target_std1, pre_str = GaussOperation.Woodbury(K, y_new, Lambda1, alpha1,
                                                                                          target_mean1, target_std1,
                                                                                          pre_str)
        ...
        target_mean2 = old_model.getTargetMean2()
        target_std2 = old_model.getTargetStd2()
        pre_str = old_model.getSampleSize()
        for k in range(0, length):
            y_new = update_data[k, -1].astype(float)
            y_new = np.log(y_new)
            ...
            sample_data = [update_data[k, 0:-1]]
            D_star = GaussOperation.gp_k_cplt(sample_data, old_model.getIdxSr(), old_model.getFeatureType())
            K_star = GaussOperation.kernel_gauss_gp_cplt(D_star, theta2, eta2)
            K = K_star.transpose()

            alpha2, Lambda2, target_mean2, target_std2, pre_str = GaussOperation.Woodbury(K, y_new, Lambda2, alpha2,
                                                                                          target_mean2, target_std2,
                                                                                          pre_str)
        old_model.setAlpha1(alpha1)
        old_model.setAlpha2(alpha2)
        old_model.setLambda1(Lambda1)
        old_model.setLambda2(Lambda2)
        old_model.setTargetMean1(target_mean1[0][0])
        old_model.setTargetMean2(target_mean2[0][0])
        old_model.setTargetStd1(target_std1[0][0])
        old_model.setTargetStd2(target_std2[0][0])
        old_model.setSamplesize(pre_str)
    ...
    def cross_validate(self, data_path, target_name, feature_name, feature_type, feature_weight, start=0, length=-1,
                       training_set_perc=0.5, max_iter=100, rglr=1e-1, learning_rate=0.005,
                       model_path='cross_validate_model.txt'):
        ...
        model = self.train(data_path, target_name, feature_name, feature_type, feature_weight, start, train_len,
                           model_path, max_iter, rglr, learning_rate)
        model.save(model_path)
        ...
        test_target = np.array(raw_data.loc[train_len: length - 1, target_name].astype(float)).reshape(
            length - train_len, 1)
    ...
    def gp_k_cplt2(self, data, data_sr, feature_type):
    ...
    def kernel_gauss_gp_cplt2(self, dist, theta, delta):
        Len = len(dist)
        m, n = dist[0].shape
...
class GPRModel:
    def predict(self, predict_file, start=0, length=-1, result_file='result.txt'):
        ...
        D_star = GaussOperation.gp_k_cplt(data, self.getIdxSr(), self.getFeatureType())
        theta = np.exp(self.getTheta2())

        num_of_feature = len(self.getFeatureName())

        eta = np.exp(self.getEta2())
        K_star = GaussOperation.kernel_gauss_gp_cplt(D_star, theta, eta)

        f_mean = np.dot(K_star, self.getAlpha2())
        f_mean = self.getTargetStd2() * f_mean + self.getTargetMean2()
        ...
        if Idx_redo != []:
            ...
            D_star = GaussOperation.gp_k_cplt(data_retry, self.getIdxSr(), self.getFeatureType())
    ...
    def predictSingle(self, sample, sample_id, result_file='singleResult.txt'):
        ...
        D_star = GaussOperation.gp_k_cplt(test_data, self.getIdxSr(), self.getFeatureType())
        theta = np.exp(self.getTheta2())
    ...
    def predictSingle_2(self, sample, sample_id, result_file='singleResult.txt'):
        # tic = time.time()
        ...
        f_mean_1 = np.dot(K_star_1, self.getAlpha1())
        f_mean_1 = self.getTargetStd1() * f_mean_1 + self.getTargetMean1()

        f_mean_2 = np.dot(K_star_2, self.getAlpha2())
        f_mean_2 = self.getTargetStd2() * f_mean_2 + self.getTargetMean2()
    ...
    def update_online(self, sample, y_new):
        ...
        alpha1, Lambda1, target_mean1, target_std1, pre_str = GaussOperation.Woodbury(K_star_1, y_new,
                                                                                      self.getLambda1(),
                                                                                      self.getAlpha1(),
                                                                                      self.getTargetMean1(),
                                                                                      self.getTargetStd1(),
        ...
        self.setTargetStd1(target_std1)

        K_star_2 = K_star_2.transpose()

        alpha2, Lambda2, target_mean2, target_std2, pre_str = GaussOperation.Woodbury(K_star_2, np.log(y_new),
                                                                                      self.getLambda2(),
                                                                                      self.getAlpha2(),
                                                                                      self.getTargetMean2(),
                                                                                      self.getTargetStd2(),
        ...
        self.setTargetStd2(target_std2)

        self.setSampleSize(pre_str)
    ...
    def load(self, model_path):
        ...
        for k in range(0, num_of_feature):
            ...
            D_k = GaussOperation.gp_k_cplt(data, data_sr, [Type])

            K_1_k = GaussOperation.kernel_gauss_gp_cplt(D_k, 1, [delta1[k]])
            K_2_k = GaussOperation.kernel_gauss_gp_cplt(D_k, 1, [delta2[k]])
...
class GaussOperation:
    ...
    @staticmethod
    def kernel_gauss_gp_cplt(dist, theta, delta):
        Len = len(dist)
    ...
    @staticmethod
    def gp_k_cplt(data, data_sr, feature_type):
    ...
    def Woodbury(K, y_new, Lambda, Alpha, target_mean, target_std, Str):
        ...
        target_std = np.sqrt(
            float(Str) / (float(Str) + 1) * target_std ** 2 + (y_new - target_mean) ** 2 / (float(Str) + 1))
        ...
        BLK = np.dot(np.transpose(K), Lambda)
        BLK = np.dot(K, BLK)
        Nm = np.dot(Lambda, BLK)
        Dnm = 1 + np.trace(BLK)

        I = np.identity(Len, dtype=float)
        BLK2 = I - float(1) / Dnm * np.transpose(BLK)

        Lambda = Lambda - float(1) / Dnm * Nm
        Alpha = f_fgt * np.dot(BLK2, Alpha) + y_new * np.dot(Lambda, K)

        return Alpha, Lambda, target_mean, target_std, Str
...
if __name__ == '__main__':
    ...
    gpr = GPRFlowEstimator()
    model = gpr.train(input, target_name, feature_name, feature_type, feature_weight, start, length)
    # print 'batch test stage'
    model2 = GPRModel()
    model2.load('GPR_model.txt')
