...
class GPRFlowEstimator:
    def train(self, data_path, target_name, feature_name, feature_type, feature_weight=[], start=0, length=-1, model_path='GPR_model.txt', max_iter=100, rglr=1e-1, learning_rate=0.005):
        ...
        idx_sr, target1, target_mean1, target_std1, D_nm, D_mm, theta_gp1, eta_gp1, Theta_gp1, Eta_gp1, LLH1 = self.gp_training_cplt_2(data, -1, feature_type, feature_weight, max_iter, rglr, learning_rate)
        idx_sr_data, target2, target_mean2, target_std2, D_nm, D_mm, theta_gp2, eta_gp2, Theta_gp2, Eta_gp2, LLH2 = self.gp_training_cplt_2(data, -1, feature_type, feature_weight, max_iter, rglr, learning_rate, idx_sr)
        ...
        Lambda1 = np.real(inv(np.dot(np.transpose(K_nm1), K_nm1)+rglr*(K_mm1 + rglr*np.identity(len(idx_sr),dtype=float))))
        ...
        Lambda2 = np.real(inv(np.dot(np.transpose(K_nm2), K_nm2)+rglr*(K_mm2 + rglr*np.identity(len(idx_sr), dtype=float))))
        ...
        return model
    def gp_training_cplt_2(self, input, target_pos, feature_type, feature_weight, max_iter, rglr, lda_learning, idx_sr_pre=[]):
        ...
        while curr_iter < max_iter:
            ...
            for i in range(0, num_of_feature):
                ...
                eta[i] =  pre_eta[i] - lda_learning*(self.gp_likelihood_cplt2(target, D_nm, D_mm, pre_theta, pre_eta_stepsize, rglr) - Old_LLH)/stepsize

            Eta[:,curr_iter] = np.array(eta).reshape(num_of_feature,1)[:,0]
            
            theta = pre_theta - lda_learning*(self.gp_likelihood_cplt2(target, D_nm, D_mm, pre_theta+stepsize, pre_eta, rglr) - Old_LLH)/stepsize
            Theta.append(theta)
    ...
    def update(self, data_path, model_path, start =  0, length = -1, overwrite = True): 
         ...
         for k in range(0,length):
             ...
             K = K_star.transpose()
             alpha1, Lambda1, target_mean1, target_std1, pre_str = GaussOperation.Woodbury(K, y_new, Lambda1, alpha1, target_mean1, target_std1, pre_str)
         ...
         for k in range(0,length):
             ...
             alpha2, Lambda2, target_mean2, target_std2, pre_str = GaussOperation.Woodbury(K, y_new, Lambda2, alpha2, target_mean2, target_std2, pre_str)
...
class GPRModel: 
    ...
    def update_online(self, sample, y_new):
        ...
        alpha1, Lambda1, target_mean1, target_std1, pre_str = GaussOperation.Woodbury(K_star_1, y_new, self.getLambda1(), self.getAlpha1(), self.getTargetMean1(), self.getTargetStd1(), self.getSampleSize()) 
        ...
        alpha2, Lambda2, target_mean2, target_std2, pre_str = GaussOperation.Woodbury(K_star_2, np.log(y_new), self.getLambda2(), self.getAlpha2(), self.getTargetMean2(), self.getTargetStd2(), self.getSampleSize()) 
